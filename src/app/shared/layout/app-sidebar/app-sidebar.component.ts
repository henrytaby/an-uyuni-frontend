...
import { Component, ElementRef, QueryList, ViewChildren, ChangeDetectorRef, inject, OnInit, OnDestroy, AfterViewInit } from '@angular/core';
...
export class AppSidebarComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  navItems: NavItem[] = [
    ...
    },
    {
      ...
      subItems: [
        { name: "Form Elements", path: "/forms", pro: false }
      ],
    },
    {
      name: "Tables",
      ...
      subItems: [
        { name: "Tables Overview", path: "/tables/overview", pro: false },
  ...
  ngAfterViewInit() {
    // Initial load - calculation moved here to avoid forced reflow before view init
    // Wrapped in setTimeout to prevent ExpressionChangedAfterItHasBeenCheckedError (NG0100)
    setTimeout(() => {
      this.setActiveMenuFromRoute(this.router.url);
    });
  }
