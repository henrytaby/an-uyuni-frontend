...
*   **Atomic Tracking (`Set<string>`)**: En lugar de un contador simple (+1/-1), rastreamos URLs exactas en un `Set`. Esto hace imposible que el contador se desincronice.
...
*   **Fail-Safe**: Un temporizador de seguridad resetea el estado si una petición se queda "colgada" por más de 10 segundos.
...
**Ciclo de Navegación:**
1.  **NavigationStart**:
    *   Activa `isNavigating = true` (Muestra Skeleton).
    *   Llama a `resetLoader()` (Limpia cualquier estado HTTP residual de la página anterior).
2.  **NavigationEnd / Cancel / Error**:
